    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <Link href="/tech/inventory" className="text-sm text-green-600 hover:text-green-400">
              View All
            </Link>
          </div>
        </CardHeader>
        <CardContent>
          {inventory.length === 0 ? (
            <p className="text-slate-400 text-sm">No inventory assigned</p>
          ) : (
            <div className="flex flex-wrap gap-3">
              {inventory.slice(0, 4).map((inv) => (
                <div key={inv.id} className="bg-slate-950 px-3 py-2 rounded-lg">
                  <div className="text-sm font-medium text-slate-100">{inv.item.name}</div>
              ...
              {inventory.length > 4 && (
                <div className="bg-slate-950 px-3 py-2 rounded-lg text-sm text-slate-400">
              ...
              <Card hoverable className="mb-0">
                <div className="flex justify-between items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-slate-100">{job.clientName}</div>
      ...
      {jobs.filter(j => j.status === 'complete').length > 0 && (
        <div className="text-center">
          <Link href="/tech/jobs?status=complete" className="text-sm text-slate-400 hover:text-slate-300">
