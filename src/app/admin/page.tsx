    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="border-l-2 border-l-blue-500">
          <CardContent>
            <div className="text-3xl font-bold text-slate-100 animate-count-up">{activeJobs.length}</div>
            <div className="text-sm text-slate-400">Active Jobs</div>
          </CardContent>
        </Card>
        <Card className="border-l-2 border-l-teal-500">
          <CardContent>
            <div className="text-3xl font-bold text-teal-400 animate-count-up">{pendingReview.length}</div>
            <div className="text-sm text-slate-400">Pending Review</div>
          </CardContent>
        </Card>
        <Card className="border-l-2 border-l-green-500">
          <CardContent>
            <div className="text-3xl font-bold text-green-400 animate-count-up">{completedToday.length}</div>
            <div className="text-sm text-slate-400">Completed Today</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent>
            <div className={`text-3xl font-bold ${lowStockItems.length > 0 ? 'text-red-400 animate-count-up' : 'text-slate-100 animate-count-up'}`}>
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full bg-teal-900/200 animate-pulse ring-2 ring-teal-500/30" />
                <div>
                  <div className="font-semibold text-slate-100">
                    {pendingReview.length} job{pendingReview.length > 1 ? 's' : ''} awaiting your review
                  </div>
                  <div className="text-sm text-slate-400">These jobs need approval before they can be marked complete</div>
                </div>
              </div>
          ...
          <div className="flex flex-wrap gap-4">
            {(['assigned', 'on_the_way', 'in_progress', 'pending_review', 'complete'] as JobStatus[]).map(status => (
              <div key={status} className="flex items-center gap-2">
                ...
                <span className="text-sm text-slate-400">{jobsByStatus[status] || 0}</span>
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <Link href="/admin/jobs" className="text-sm text-blue-600 hover:text-blue-400">
              View All
            </Link>
          </div>
        </CardHeader>
              ...
              {activeJobs.slice(0, 5).map((job) => {
                ...
                return (
                  <Link
                    ...
                    className="block p-3 rounded-lg border border-slate-800 hover:border-blue-500 hover:bg-blue-900/20 transition-colors"
                  >
                    <div className="flex justify-between items-start gap-2">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          />
                          <span className="font-medium text-slate-100 truncate">{job.clientName}</span>
                          <span className="text-xs text-slate-500">{job.jobNumber}</span>
                        </div>
                        ...
                        {tech && (
                          <div className="flex items-center gap-1.5 mt-1">
                            ...
                            />
                            <span className="text-xs text-slate-500">{tech.fullName}</span>
                          </div>
                        )}
                      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <Link href="/admin/techs" className="text-sm text-blue-600 hover:text-blue-400">
              Manage
            </Link>
          </div>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {techs.map((tech) => {
              ...
              return (
                <div key={tech.id} className="flex justify-between items-center p-3 rounded-lg bg-slate-950">
                  <div className="flex items-center gap-3">
                    <div
                  ...
                  <div className="text-right">
                    <div className="text-sm font-medium text-slate-100">{techJobs.length} jobs</div>
                    {inProgress > 0 && (
